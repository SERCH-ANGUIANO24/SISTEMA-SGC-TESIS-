...
<style>
    ...
    .btn-registrar {
        ...
        box-shadow: 0 4px 14px rgba(128,0,0,.3);
                    ...
                    >
                        ...
                        <td data-label="Acciones">
                            @if(auth()->user()->isSuperAdmin())
                                {{-- Superadmin: todas las acciones sobre cualquier cuenta --}}
                                <form
                                    method="POST"
                            ...
                            @else
                                {{-- Admin: solo puede ver, sin acciones --}}
...
{{-- MODAL: Registrar Usuario (Bootstrap, solo para registro) --}}
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:520px;">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-form-group">
                        ...
                        <select name="proceso" id="modalProceso" class="modal-select @error('proceso') is-invalid @enderror" required>
                            ...
                            {{-- Procesos personalizados agregados dinámicamente --}}
                            @isset($procesosCustom)
                                @if($procesosCustom->count())
                                    <option disabled style="color:#aaa;font-size:.8rem;">── Procesos personalizados ──</option>
                                    @foreach($procesosCustom as $pc)
                                        <option value="{{ $pc->proceso }}"
                                            data-depto-custom="{{ $pc->departamento }}"
                                            {{ old('proceso')==$pc->proceso?'selected':'' }}>
                                            {{ $pc->proceso }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        {{-- Campos visibles solo cuando se elige "Otro" --}}
                ...
                {{-- Sección: procesos personalizados existentes (solo superadmin) --}}
                @if(auth()->user()->isSuperAdmin())
                    @isset($procesosCustom)
                        @if($procesosCustom->count())
                        <div style="border-top:1px solid #f0e8e8;padding-top:1rem;margin-top:.5rem;">
                            <p style="font-size:.82rem;font-weight:600;color:#800000;margin-bottom:.6rem;">
                                <i class="bi bi-list-check"></i> Procesos personalizados
                            </p>
                            @foreach($procesosCustom as $pc)
                            <div style="display:flex;align-items:center;justify-content:space-between;background:#faf8f8;border:1px solid #f0e8e8;border-radius:8px;padding:.45rem .75rem;margin-bottom:.4rem;">
                                <div>
                                    <span style="font-weight:600;font-size:.85rem;color:#2a1a1a;">{{ $pc->proceso }}</span>
                                    <span style="color:#aaa;font-size:.8rem;margin:0 .35rem;">·</span>
                                    <span style="font-size:.82rem;color:#666;">{{ $pc->departamento }}</span>
                                </div>
                                <form method="POST" action="{{ route('admin.procesos.destroy', $pc->id) }}"
                                    onsubmit="return confirm('¿Eliminar el proceso «{{ addslashes($pc->proceso) }}»?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" style="background:none;border:none;color:#cc5500;cursor:pointer;font-size:.85rem;padding:.2rem .4rem;" title="Eliminar">
    ...
    function filterModalDepto() {
        ...
        if (!esOtro) {
            ...
            modalDepto.querySelectorAll('option').forEach(opt => {
                ...
                if (deptoCustom) {
                    // Proceso custom: mostrar solo su departamento correspondiente
                    opt.style.display = (opt.value === deptoCustom) ? '' : 'none';
                } else {
                    // Proceso normal: filtrar por data-proceso
                    const allowed = (opt.getAttribute('data-proceso') || '').split(',');
                    opt.style.display = (!sel || allowed.includes(sel)) ? '' : 'none';
                }
            });

            // Si es proceso custom, preseleccionar su departamento automáticamente
            if (deptoCustom) {
                modalDepto.value = deptoCustom;
            }
        }
    }
...
// Toggle contraseñas en overlay editar
...
// Validar que contraseñas coincidan en overlay editar
...
// Escape cierra overlay editar
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') cerrarOverlayEditar();
});

// confirm() nativo — activ/desactiv
